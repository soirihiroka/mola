...
use crate::ui::ui_controller::SceneViewFocus;
...
impl Plugin for CameraControllerPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            // run_camera_controller.run_if((egui_wants_any_keyboard_input)),
...
pub fn run_camera_controller(
    ...
    mut view_focus: ResMut<SceneViewFocus>,
) {
    if !view_focus.0 {
    ...
    // let egui_blocking = egui_wants_any_pointer_input(egui_wants_input);
    ...
    let cursor_grab = (*mouse_cursor_grab || *toggle_cursor_grab) && view_focus.0;
    ...
    if cursor_grab_change {
        info!("grab changed to {cursor_grab}");
        if cursor_grab {
            for mut window in &mut windows {
                if !window.focused {
                    continue;
